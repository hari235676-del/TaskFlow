...
function filtered() {
  return tasks.filter(t => {
    const matchS = !searchQuery || t.title.toLowerCase().includes(searchQuery) || (t.desc||'').toLowerCase().includes(searchQuery);
...
function formatDue(ds) {
  ...
  const label = diff===0?'Today':diff===1?'Tomorrow':diff===-1?'Yesterday':diff<0?`${Math.abs(diff)}d ago`:diff<=7?`In ${diff}d`:due.toLocaleDateString('en-US',{month:'short',day:'numeric'});
  ...
  return `<div class="card" draggable="true" data-id="${t.id}">
    <div class="card-top">
      ...
      <div class="card-actions">
        <button class="card-btn" onclick="markDone('${t.id}')" title="${t.col === 'done' ? 'Undo' : 'Mark as Done'}">
          ${t.col === 'done' ? '↩' : '✓'}
        </button>
